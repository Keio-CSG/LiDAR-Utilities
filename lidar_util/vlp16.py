...
def parse_packet_vlp16_dual(timestamp: float, d: bytes, offset: int, last_azimuth=None):
    """
    保存したpacketのdataをパースする
    ...
    - 偶数のdata_blockにはstrongest、奇数にはlastのデータが入っている
    - channel data Aにazimuthの角度のデータが入っている
    - channel data Bにazimuth+1の角度のデータが入っている
    """
    data = d[offset:offset+1206]
    # packet内のtimestampは3600秒しか測れないので捨てる
    timestamp_in_packet, factory = struct.unpack_from("<IH", data, offset=1200)
    assert factory == 0x2239, hex(factory)  # 0x22=VLP-16, 0x39=Dual Return
    sequence_index = 0
    prev_azimuth = last_azimuth
    cut_point = None
    points = []
    for data_block in range(0, 12):
        offset_inside = data_block * 100
        is_last = data_block % 2 == 1 # 奇数ブロックはlast
        # Data Blockの開始フラグと方位角(azimuth)
        flag, block_azimuth = struct.unpack_from("<HH", data, offset_inside)
        assert flag == 0xEEFF, hex(flag)
        # Data Blockには2つの方位角のデータが48 byteずれて入っている
        for step in range(2):
            ...
            for channel in range(16):
                distance = channel_data_list[channel*2]
                reflectivity = channel_data_list[channel*2+1]
                offset_time_sec = (SEQUENCE_TIME_US * sequence_index + PULSE_TIME_US * channel) / 1000000.0
                if distance != 0:
                    points.append(calc_point(
                        distance, azimuth, channel, timestamp + offset_time_sec,
                        reflectivity, LASER_ANGLES, DISTANCE_RESOLUTION
